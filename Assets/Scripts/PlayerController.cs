...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private List<GameObject> ganonPoseList = new List<GameObject>();
    [SerializeField] private List<GameObject> ganonShadowList = new List<GameObject>();
    ...
    void Update()
    {
        if ((Input.GetKeyDown("w") || Input.GetKeyDown("up")) && !crounching && timer < changeWindow)
        {
            foreach (GameObject pose in ganonPoseList)
            {
                ...
                {
                    blockIndex = 2;
                    if (ganonShadowList[5].activeSelf) ganonShadowList[4].SetActive(false);
        ...
        if ((Input.GetKeyDown("a") || Input.GetKeyDown("left")) && columnNum == 2 && timer < changeWindow)
        {
            foreach (GameObject pose in ganonPoseList)
            {
                ...
                {
                    blockIndex = 6;
                    if (ganonShadowList[7].activeSelf) ganonShadowList[4].SetActive(false);
        ...
        {
            if (shadow.activeSelf && shadow.GetComponent<Image>().color.a != 0)
                DOTween.To(() => shadow.GetComponent<Image>().color, x => shadow.GetComponent<Image>().color = x, new Color(0, 0, 0, 0), 2);
            if (shadow.GetComponent<Image>().color.r == 0)
            {
                shadow.GetComponent<Image>().color = new Color(255, 255, 255, 255);
